...
describe('UsersService role selection', () => {
  ...
  it('keeps consumer as the only role for a consumer account', async () => {
    ...
    expect(update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ roles: ['CONSUMER'], onboardingStep: 'COMPLETE' }) }));
  });

  it('allows consumption while making seller types mutually exclusive', async () => {
    await service.selectType(user, 'BUSINESS');
    expect(update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ roles: ['CONSUMER', 'BUSINESS'], onboardingStep: 'BUSINESS_DETAILS_REQUIRED' }) }));
