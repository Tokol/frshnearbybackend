...
export class AuthService {
  ...
  async authenticate(header?: string) {
    ...
    try { decoded = await this.firebase.verify(header.slice(7)); }
    ...
    const user = await this.prisma.user.upsert({
      ...
      create: {
        ...
        onboardingStep: decoded.email_verified ? 'PROFILE_REQUIRED' : 'EMAIL_VERIFICATION_REQUIRED',
      },
      update: {
        email: decoded.email,
        emailVerified: decoded.email_verified ?? false,
    ...
    if (user.status === 'DELETED') throw new ForbiddenException('Account deleted');
    return user;
  }
}
